...
for a repository by running five scoring components through a language adapter.
...
| `compute.py` | Composite score orchestrator -- calls all five components. |
...
Five independent scoring components, each returning a 0-100 sub-score.
...
| `test_config.py` | Config loading, validation, and default-merging tests. |
